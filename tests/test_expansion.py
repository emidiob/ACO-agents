...
class ExpansionTests(unittest.TestCase):
 ...
 def test_expected_release_coverage(self):
  self.assertEqual(len(self.c['agents']),349);self.assertEqual(self.c['skill_count'],16)
  self.assertEqual(self.c['offices']['product-office']['count'],53)
 ...
 def test_all_roles_have_short_action_contract(self):
  for k,a in self.c['agents'].items():
   self.assertIn('INTERACTION AND ACTION CONTRACT',(ROOT/a['path']).read_text(),k)
 def test_all_native_profiles_include_short_action_contract(self):
  for k,a in self.c['agents'].items():
   self.assertIn('INTERACTION AND ACTION CONTRACT',tomllib.loads((ROOT/a['codex_path']).read_text())['developer_instructions'],k)
 ...
 def test_runtime_helpers_present(self):
  for name in ['planning.py','finance.py','production.py']:
