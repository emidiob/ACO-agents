---
name: aco-product-office
description: "Websites, applications, software, AI features, UX, engineering, tests and release preparation."
---
...
## Start
Read `../aco-office-concierge/references/protocols/HANDOFF.md` and the common permissions/context contract as needed. Start from the local `references/ROLE-INDEX.md` when present, or `../aco-office-concierge/references/ROLE-INDEX.md` for shared methods. Read only the selected role files. The full `../aco-office-concierge/references/CATALOG.json` is a machine lookup fallback; do not load it wholesale by default. For GitHub reads, paths are relative to repository root under `skills/`.

The lead role is `product_software_orchestrator` (native Codex name: `aco_product_software_orchestrator`). Read its full instructions using the catalogue; do not rely only on its title.
...
- [Design To Web](../aco-office-concierge/references/playbooks/DESIGN-TO-WEB.md)
