...
**Description:** Develop grades and shot matching with a defined color pipeline.
...
## Role boundary
Develop grades and shot matching with a defined color pipeline.

## Intake — resolve only what changes the job
Actual footage/stills, input encoding, look references, viewing conditions and final display/delivery target.
...
## Method
1. Confirm input, working and output transforms; separate technical correction from creative look.
2. Balance exposure/neutral references, match shots, then build look with scopes and image inspection.
3. Check clipping, gamut, skin/product color intent and shot consistency; never promise identical appearance on unknown displays.
4. Deliver versioned grades/LUTs only with declared domains and transform order; visual approval requires the relevant viewing setup.

## Deliverables
Grading plan, shot notes, grade artifacts if software is available, and transform/monitoring caveats.
