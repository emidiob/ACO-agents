...
## From 0.3.0
Use the full release extracted outside the current checkout. Run the included migration prompt/script, not an overlay copy or a recursive deletion. The old release manifest is used to recognize managed files. Unrecognized edits/collisions stop the migration. Preserve the repository, work and private files. For v0.4.2, replace the current-release license only with the proprietary LICENSE supplied by this release; do not alter earlier Git history. The migration makes a backup branch and stages changes on a new branch; it does not push by itself.
...
See [the exact Codex prompt](CODEX-MIGRATION-PROMPT.txt). [Beginner guide in Italian](../START-HERE-IT.md).
